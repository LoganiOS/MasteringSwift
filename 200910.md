...
- <details>
  ...
  - 조건문과 반복문을 사용해 2부터 num까지 숫자 중, 소수만 쏙 빼서 배열에 짚어넣는 함수를 만들어야 한다.
  ...
  - ```swift
    ...
    4.  if문은 조건이 true일때만 실행이 된다.
       priems의 i번째 요소가 true이면 if문이 실행이 되는데, 아직은 primes배열의 모든 요소가 true이기 때문에, 우선은 실행이 되며 primeNumber 배열에 반복상수 i를 순서대로 추가한다.
    ...
    9.  다시 반복구문이 실행되서 ④번으로 돌아와 if문을 평가할 땐, ⑥에서 바뀐 false로 평가된 if문은 실행되지 않으며, 다음 반복상수를 평가하고 true일때만 if문이 실행된다.
